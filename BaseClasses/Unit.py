...
class Unit():
  ...
  #region init and destroy
    ...
    def destroy(self, task=None):
        ...
        try:
            ...
            if self in unitManager().Teams[self.Team]:
                raise
        self.__del__()
        #if task:
        #    return Task.cont
  ...
  #region Interaction
    def interactWith(self, hex:'Hex._Hex'): #TODO:OVERHAUL
        ...
        if hex.unit:
            self.lookAt(hex)
            if not ( hex.unit() is self ): #TODO: Teams check
                self.attack(hex.unit())
            return False
        else:
  ...
  #region Movement
    def moveToHex(self, hex:'Hex._Hex', animate= True):
        ...
        if hex.unit:
            ...
            if not hex.unit: #TODO: We have a serious problem when this occurs. What do we do in that case?
                raise Exception("Could not assign unit to Hex")
            if hex.unit() != self: #TODO: We have a serious problem when this occurs. What do we do in that case?
                raise Exception("Could not assign unit to Hex")
    ...
    def moveTo(self, hex:'Hex._Hex'):
        if not self._navigable(hex):
            ...
            if not path or cost > self.MovePoints:
                ...
                if not hex.unit: #TODO: We have a serious problem when this occurs. What do we do in that case?
                    raise Exception("Could not assign unit to Hex")
                if not hex.unit() == self: #TODO: We have a serious problem when this occurs. What do we do in that case?
                    raise Exception("Could not assign unit to Hex")
                return True
  ...
  #region Display Information
    def diplayStats(self, display=True):
        if display:
            ...
            """)
            window().UnitStatDisplay.Text.setText(text)
        else:
            window().UnitStatDisplay.Text.setText("No unit selected")
